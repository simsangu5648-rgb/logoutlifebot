...
const MONTHLY_CHALLENGE_SHEET_TAB_NAME = process.env.MONTHLY_CHALLENGE_SHEET_TAB_NAME || "매달챌린지기록";
...
// 매달 30일 챌린지(금딸챌린지) 체크인 1건을 시트에 한 줄 추가합니다.
// 리부트 챌린지 기록과 마찬가지로 실패해도 예외를 던지지 않으므로, 호출하는
// 쪽에서 await 없이 .catch()만 붙여서 쓰는 걸 권장합니다.
// event: { discordUserId, label, monthKey, successDays, completed, completedMonthsTotal, crisisDetected, relapse }
async function appendMonthlyChallengeEvent(event) {
  if (!isConfigured()) {
    if (!warnedMissingConfig) {
      warnedMissingConfig = true;
      console.warn(
        "[구글시트] REBOOT_SHEET_ID / GOOGLE_SERVICE_ACCOUNT_EMAIL / GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY 중 " +
          "설정 안 된 게 있어서 매달챌린지 기록 기능을 건너뜁니다."
      );
    }
    return;
  }
  try {
    const sheets = await getSheetsClient();
    await ensureHeader(sheets, MONTHLY_CHALLENGE_SHEET_TAB_NAME, [
      "시각(KST)", "디스코드ID", "닉네임", "대상월", "이번달 성공일수", "이번달 완주", "누적 완주 개월", "재발", "위기감지",
    ]);
    await sheets.spreadsheets.values.append({
      spreadsheetId: SHEET_ID,
      range: `${MONTHLY_CHALLENGE_SHEET_TAB_NAME}!A:I`,
      valueInputOption: "RAW",
      insertDataOption: "INSERT_ROWS",
      requestBody: {
        values: [
    ...
    console.error("[구글시트] 매달챌린지 기록 실패 (봇 동작에는 영향 없음):", e.message || e);
  }
}

module.exports = { appendRebootEvent, appendSalesEvent, appendConfessionEvent, appendMonthlyChallengeEvent, isConfigured };
